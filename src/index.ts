 ...
 * only edits reaches `choisy-video-kit/editor`, which is pure TypeScript that touches no bridge, and
 * a mandatory peer would install a native bridge into the `node_modules` of every such host.
 * `@choisy/video-editor` is exactly that host and imports `choisy-video-kit/editor` only.
 */
