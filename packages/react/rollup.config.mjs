...
const core = 'capacitor-video-kit';
 ...
 * installs `capacitor-video-kit/react` - and 12.2 MB of that is `@ts-morph/common`, a TypeScript
 ...
 * is inlined. React stays a peer dependency and `capacitor-video-kit` stays a dependency, because
 ...
 * `import { type VeAlertCustomEvent } from "capacitor-video-kit"` - the bare package name, hard coded,
 * with no option to point it anywhere else. Its own Angular target writes
 * `capacitor-video-kit/dist/components` for the same type, which is the specifier that is actually
 ...
 * Capacitor plugin, so `capacitor-video-kit` resolves to `plugin/esm/plugin.d.ts`, which exports none
