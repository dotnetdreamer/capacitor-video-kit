# capacitor-video-kit/react

React bindings for the Capacitor Video Kit editor.

```sh
npm install ./capacitor-video-kit-1.3.0.tgz ./capacitor-video-kit/react-1.3.0.tgz
```

```tsx
import { VeSpinner } from 'capacitor-video-kit/react';
...
`capacitor-video-kit` is a peer dependency pinned to the exact version of this package, because the two
...
of `node_modules/capacitor-video-kit/dist/components/assets` and name it once at startup:

```ts
import { setEditorAssetPath } from 'capacitor-video-kit/ui';
...
two types the wrappers name from them. React, `react-dom` and `capacitor-video-kit` stay external,
...
`capacitor-video-kit/vue` is not bundled, because `@stencil/vue-output-target` has no dependencies
