# @capacitor-video-kit/core/react

React bindings for the Capacitor Video Kit editor.

```sh
npm install ./@capacitor-video-kit/core-1.3.0.tgz ./@capacitor-video-kit/core/react-1.3.0.tgz
```

```tsx
import { VeSpinner } from '@capacitor-video-kit/core/react';
...
`@capacitor-video-kit/core` is a peer dependency pinned to the exact version of this package, because the two
...
of `node_modules/@capacitor-video-kit/core/dist/components/assets` and name it once at startup:

```ts
import { setEditorAssetPath } from '@capacitor-video-kit/core/ui';
...
two types the wrappers name from them. React, `react-dom` and `@capacitor-video-kit/core` stay external,
...
`@capacitor-video-kit/core/vue` is not bundled, because `@stencil/vue-output-target` has no dependencies
